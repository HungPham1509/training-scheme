...
class HigherOrderItems extends Component {
  render() {
    ...
    if (this.props.label !== "Thống Kê") {
      list = !this.props.collapsed ? (
        <ul className={classes.itemsContainer}>{listItems}</ul>
      ) : (
        <ul className={classes.itemsContainerOff}>{listItems}</ul>
      );

      if (!this.props.toggleOn) {
        toggleIcon = (
          <img className={classes.ToggleOff} src={ToggleIcon} alt="icon" />
        );
      } else {
        toggleIcon = (
          <img className={classes.ToggleOn} src={ToggleIcon} alt="icon" />
        );
      }
    }
    ...
    if (this.props.label === "Thống Kê") {
      higherOrderItem = !this.props.collapsed ? (
        <NavLink
          to="/statistics"
      ...
      higherOrderItem = !this.props.collapsed ? (
        ...
        <div className={classes.containerOff}>
          <img src={this.props.icon} alt="icon" />
          <div
            className={classes.labelContainerOff}
            style={
              this.props.toggleOn
                ? { visibility: "unset" }
                : { visibility: "hidden" }
